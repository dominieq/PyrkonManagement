...
void mainLoop(void)
{
	...
	while (!end)
	{
		...
		// 	pthreac_t new_thread;
		// 	pthread_mutex_lock( &odpowiadam_na_stare_wiadomosci);
		// 	pthread_create(&new_thread, NULL, odpowiedz_na_stare_wiadomosci(0), 0);
		// 	pthread_mutex_lock( &odpowiadam_na_stare_wiadomosci);
		...
		case BEFORE_PYRKON:
		{
			println("STATUS: BEFORE_PYRKON: (%d)\n", get_pyrkon_number());
			...
			/* Process was allowed to enter to so it changes it's state. */
			set_state(ON_PYRKON);
			println("STATUS: ENTERING_PYRKON: (%d)\n", get_pyrkon_number());
			println("Entering Pyrkon.\n");
...
//			pthread_mutex_unlock(&allowing_lecture);
		...
		case ON_PYRKON:
		{
			...
			// pthread_mutex_unlock(&modify_exited_from_pyrkon);
			set_state(AFTER_PYRKON);
			println("STATUS: EXITING_PYRKON: (%d)\n", get_pyrkon_number());
		...
		case AFTER_PYRKON:
		{
			...
			pthread_mutex_lock( &odpowiadam_na_stare_wiadomosci);
			println("STATUS: ODPOWIADAM NA STARE WIADOMOSCI\n");
			pthread_create(&new_thread, NULL, odpowiedz_na_stare_wiadomosci, 0);
			pthread_mutex_lock( &odpowiadam_na_stare_wiadomosci);
			println("STATUS: SKONCZYLEM ODPOWIADAC\n");
			pthread_mutex_unlock( &odpowiadam_na_stare_wiadomosci);
			...
			println("STATUS: WAITING_FOR_NEW_PYRKON: (%d)\n", get_pyrkon_number());
			czy_moge_wyjsc();
			...
			set_state(BEFORE_PYRKON);
			println("po zmianie stanu po czy_moge_wyjsc\n");
			pthread_mutex_unlock(&on_pyrkon_mutex);
			println("Opening semaphore on_pyrkon_mutex AP.\n");
...
void allow_pyrkon(packet_t *message)
{
	...
	{	
		...
		println("S-FOR CZEKA [%d].\n", newP->src);
		push_pkt_pyrkon(newP, 0);
	}
	pthread_mutex_unlock( &my_clocks_edit);
	// pthread_mutex_unlock(&allowing_pyrkon);
	...
	case 0:
		while (TRUE) {
			...
			if (pakiet == NULL) {
				...
				// pthread_t new_thread;
				// pthread_create(&new_thread, NULL, (void *)handlers[WANT_TO_ENTER], pakiet);
				println("SEGMENTATION.\n");
				println("FOR PO [%d].\n", newP->src);
				println("KKK: %d, %d, %d, %d, %d \n", newP->ts, newP->data, newP->dst, newP->src, newP->pyrkon_number)
				// pakiet = NULL;
				free(pakiet);
			}
			println("SEGMENTATION-S\n");
		}
		println("SEGMENTATION!!.\n");
